...
parser.add_argument('-download_file','-df', help="爬虫文件夹路径, 可选参数， 默认值为Spiders", default="./")
...
#
# path = "./wen.log" # 日志文件地址
# logging.basicConfig(filename=path, level=logging.INFO, format='%(levelname)s:%(asctime)s:%(message)s', ) # 日志文件配置
#
# url = ["https://www.shicimingju.com/"] # 起始页url
# url_index = re.findall(r'[http|https]+://.*?/', str(url[0]))
#
# d = 1 # 爬虫深度
# c = 32 # 线程数
# key = "山中"
...
# # 请求函数
# def get_request(url):
#     ex = conn.sadd("html_url", url)
#     if ex == 1:
#         try:
#             session = requests.session() # 请求session
#             page_text = session.get(url=url, headers=header) # 发起get请求
#             # print(page_text)
#             page_text.encoding = 'utf-8' # 设置响应字符集
#             logging.info("已请求的url:%s"%url) # 添加日志信息
#         except Exception as a:
#             logging.error("请求错误，错误类型为：%s, 错误url为%s"%(a, url))
#         return page_text.text # 返回响应
#     return 0
#
#
# rt_list = []
# # 页面解析
# def get_text(texts):
#     key_list = re.findall(key, str(texts))
#     if key_list:
#         with open(r"D:\Spyder\1.html",encoding='utf-8', mode='a') as f:
#             f.write(texts)
#         # print(key_list)
#     tag = re.findall(r'<a href=".*?"', str(texts))
#     url_list = []
...
#
#         else:
#             url_list.append(url_index[0] + i[9:-1])
#     return url_list
#
#
#
...
#     result_data = pool.map(get_text, result_list)
#     # print(result_data)
#     # rt_list.extend(result_data)
#     d -= 1
#     for i in result_data:
#         abc(i, d)
#     # return result_data
